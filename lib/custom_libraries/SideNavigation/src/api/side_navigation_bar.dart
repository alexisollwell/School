...
class SideNavigationBar extends StatefulWidget {
  ...
  final SideNavigationBarItem itemBottom;
  ...
  State<SideNavigationBar> createState() => SideNavigationBarState();

  /// Access [_SideNavigationBarState] from outside based on [context]
  static SideNavigationBarState of(final BuildContext context) {
    final SideNavigationBarState? state =
        context.findAncestorStateOfType<SideNavigationBarState>();
    if (state == null) {
...
class SideNavigationBarState extends State<SideNavigationBar> {
  ...
  List<Widget> _generateItems() {
    List<Widget> _items = widget.items
        ...
        .toList();
    //_items.insert(widget.items.length-2, Spacer());
    //_items.add(SizedBox(height: 10,));
    return _items;
  }

  Widget _generateItemsBottom() {
    return SideNavigationBarItemWidget(
      itemData: widget.itemBottom,
      onTap: (value)=>widget.itemBottom.onTap!(),
      itemTheme: _theme.itemTheme,
      index: 1,
      expanded: _expanded,
    );
